...
import (
	...
	"github.com/iff-guardian/platform/internal/auth"
...
func main() {
	...
	// Initialize metrics
	metricsCollector := metrics.NewCollector("auth_service")
	...
	// Create auth service
	authService := auth.New(cfg, logger, metricsCollector, db)
	...
	router.Use(auth.MetricsMiddleware(metricsCollector))
	...
	go func() {
		logger.Info("Starting auth service", "port", cfg.Port)
